...
const summarizeBalances = async (balance: WalletBalance) => {
  ...
  const assets = await Promise.all(
    balance.assets.map(({ assetId, amount }) =>
      wallet.assetManager.getAssetDetails(assetId).then((details) => {
  ...
  return JSON.stringify([bitcoin, ...assets], null, 2);
};
...
const burnTxIds = await Promise.all(
  balance.assets.map((asset) => wallet.assetManager.burn(asset)),
);
console.log(
  `\nBurned ${burnTxIds.length} existing assets:`,
  JSON.stringify(
    burnTxIds.map((burnTxId) => `https://arkade.space/tx/${burnTxId}`),
    null,
    2,
  ),
);
...
const { arkTxId: newIssueTxId, assetId: newAssetId } =
  await wallet.assetManager.issue({
    controlAssetId,
    amount: issueAmount,
    metadata,
