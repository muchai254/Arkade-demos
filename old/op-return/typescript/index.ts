...
console.log(
  "Generated address:",
  vtxoScript.address(networks.bitcoin.hrp, operatorPubkey).encode(),
);
  ...
  if (!txs.length) {
    ...
    for (let vout = 0; vout < tx.outputsLength; vout++) {
      ...
      if (outScript?.type === "p2a") {
        console.log(
          `Found anchor output at index #${vout} with amount ${amount}`,
        );
        continue;
      }
      if (outScript && "pubkey" in outScript) {
        console.log(
          ...
          [new ArkAddress(operatorPubkey, outScript.pubkey, "ark").encode()],
        );
        continue;
      }
      ...
      if (script?.[0] === "RETURN" && script?.[1] instanceof Uint8Array) {
        ...
        if (bytes.length === 32) {
          console.log(
            ...
            [new ArkAddress(operatorPubkey, bytes, "ark").encode()],
          );
          continue;
        } else {
          console.log(
            `Found op_return output at index #${vout} with amount ${amount}`,
            [utf8.encode(bytes)],
          );
          continue;
        }
      ...
      console.error(
        `Could not decode output at index #${vout} with amount ${amount}`,
        output,
      );
    }
  }
